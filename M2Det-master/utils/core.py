...
from data import COCODetection, VOCDetection, detection_collate, preproc, CSVDataSet
...
def get_dataloader(cfg, dataset, setname='train_sets'):
    ...
    Dataloader_function = {'VOC': VOCDetection, 'COCO':COCODetection, 'CSV':CSVDataSet}
    _Dataloader_function = Dataloader_function[dataset]
    if setname == 'train_sets':
        if dataset=='CSV':
            dataset = _Dataloader_function(getattr(cfg.dataset, dataset)[setname],
                                           getattr(cfg.dataset, dataset)['classes'], cfg.CSVroot, _preproc)
        else:
        ...
        if dataset=='CSV':
            dataset = _Dataloader_function(getattr(cfg.dataset, dataset)[setname],
                                           getattr(cfg.dataset, dataset)['classes'], cfg.CSVroot)
        else:
            dataset = _Dataloader_function(cfg.COCOroot if dataset == 'COCO' else cfg.VOCroot,
